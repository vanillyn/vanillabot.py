...
class ConfigCog(commands.Cog):
    ...
    async def config(self, ctx):
        ...
        embed.add_field(
            ...
            value="`config server set prefix <prefix>` - Set prefix\n"
            "`config server set language <lang>` - Set server default language",
            inline=False,
        )
        embed.add_field(
    ...
    async def config_user(self, ctx):
        ...
        embed.add_field(
            name="Message Type",
            value="`config user set message_type <type>` - Set your message type preference (embed/text)",
            inline=False,
        )
        await ctx.send(embed=embed)

    @config_user.command(name="set")
    async def user_set(self, ctx, key: str, *, value: str):
    ...
    async def config_server(self, ctx):
        ...
        embed.add_field(
            name="Language",
            value="`config server set language <lang>` - Set language",
            inline=False,
        )
        await ctx.send(embed=embed)

    @config_server.command(name="set")
    async def guild_set(self, ctx, key: str, *, value: str):
        ...
        try:
            set_guild_config(ctx.guild.id, key.lower(), value.lower())
            embed = discord.Embed(
                title="Configuration Updated",
                description=f"`{key.lower()}` has been set to `{value.lower()}` for {ctx.guild.name}",
                color=0x00FF00,
            )
            await ctx.send(embed=embed)
        except Exception as e:
